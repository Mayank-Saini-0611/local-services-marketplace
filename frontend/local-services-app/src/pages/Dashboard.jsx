...
function ProviderStats({ stats, t }) {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {stats.activeListings > 0 && (
            <span className="text-xs font-medium text-green-600 bg-green-50 px-2 py-1 rounded-full">
              {stats.activeListings} {t('dashboard.active')}
            </span>
          )}
        </div>
      ...
      <div className="bg-white rounded-2xl p-5 lg:p-6 border border-slate-100 hover:shadow-lg transition-shadow">
        ...
        <p className="text-sm text-slate-500">{t('dashboard.completed')}</p>
      </div>
      ...
      <div className="bg-white rounded-2xl p-5 lg:p-6 border border-slate-100 hover:shadow-lg transition-shadow">
        <div className="flex items-center justify-between mb-4">
          ...
          {stats.totalReviews > 0 && (
            <span className="text-xs font-medium text-yellow-600 bg-yellow-50 px-2 py-1 rounded-full">
              {stats.totalReviews} {t('dashboard.reviews')}
            </span>
          )}
        </div>
        ...
        <p className="text-sm text-slate-500">{t('dashboard.avgRating')}</p>
      </div>
    </div>
  );
}

function CustomerStats({ stats, t }) {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      <div className="bg-white rounded-2xl p-5 lg:p-6 border border-slate-100 hover:shadow-lg transition-shadow">
        <div className="flex items-center justify-between mb-4">
        ...
        <p className="text-2xl lg:text-3xl font-bold text-slate-900 mb-1">{stats.totalBookings}</p>
        <p className="text-sm text-slate-500">{t('dashboard.totalBookings')}</p>
      </div>
      ...
      <div className="bg-white rounded-2xl p-5 lg:p-6 border border-slate-100 hover:shadow-lg transition-shadow">
        ...
        <p className="text-sm text-slate-500">{t('dashboard.reviewsGiven')}</p>
      </div>
    </div>
  );
}

function ProviderEmptyState({ onNavigate, t }) {
  return (
...
function Dashboard() {
  ...
  return (
    <div className="space-y-6 lg:space-y-8">
      ...
      <div className="relative bg-gradient-to-br from-violet-600 via-purple-600 to-indigo-700 rounded-3xl p-6 lg:p-10 overflow-hidden">
        ...
        <div className="relative">
          ...
          <h1 className="text-2xl lg:text-4xl font-bold text-white mb-2">
            {t('dashboard.hey')} {user.fullName ? user.fullName.split(' ')[0] : 'User'}! 👋
          </h1>
          <p className="text-violet-100 text-base lg:text-lg mb-6 max-w-xl">
            {user.role === 'provider'
              ? t('dashboard.providerWelcome')
      ...
      {/* STATS (Real Data Passed) */}
      {loading && <StatsLoading />}
      {!loading && stats && user.role === 'provider' && <ProviderStats stats={stats} t={t} />}
      {!loading && stats && user.role !== 'provider' && <CustomerStats stats={stats} t={t} />}

      {/* PROVIDER EMPTY STATE */}
      {!loading && user.role === 'provider' && stats && stats.totalListings === 0 && (
        <ProviderEmptyState onNavigate={() => navigate('/dashboard/my-listings')} t={t} />
      )}
